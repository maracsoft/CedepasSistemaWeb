...
<script type="text/javascript"> 
    ...
    <br>
    <form id="frmempresa" name="frmempresa" role="form" action="/editarHorario/save" class="form-horizontal form-groups-bordered" method="post" enctype="multipart/form-data">
        @csrf 

            <input type="text" class="form-control" id="codTurno" name="codTurno" value="{{ $turno->codTurno}}" hidden>
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    ...
                    <option value="{{$itemtipo->codTipoTurno}}" {{ $itemtipo->codTipoTurno==$turno->codTipoTurno ? 'selected':'' }}>{{$itemtipo->nombre}}</option>
                    @endforeach
                    </select>
                </div>
            </div>
            <div class="form-group row" id="horaInicio1">
                @if(!is_null($turno->horaInicio))
                <label class="col-sm-2 col-form-label" style="margin-left:350px;">Hora Inicio (MAÑANA):</label><div class="col-sm-3"><input type="time" class="form-control" id="horaInicioInput" name="horaInicioInput" value="{{$turno->horaInicio}}"></div>    
                @endif
            </div>
            <div class="form-group row" id="horaFin1">
                @if(!is_null($turno->horaFin))
                <label class="col-sm-2 col-form-label" style="margin-left:350px;">Hora Fin (MAÑANA):</label><div class="col-sm-3"><input type="time" class="form-control" id="horaFinInput" name="horaFinInput" value="{{$turno->horaFin}}"></div>
                @endif
            </div>
            <div class="form-group row" id="horaInicio2">
                @if(!is_null($turno->horaInicio2))
                <label class="col-sm-2 col-form-label" style="margin-left:350px;">Hora Inicio (TARDE):</label><div class="col-sm-3"><input type="time" class="form-control" id="horaInicio2Input" name="horaInicio2Input" value="{{$turno->horaInicio2}}"></div>
                @endif
            </div>
            <div class="form-group row" id="horaFin2">
                @if(!is_null($turno->horaFin2))
                <label class="col-sm-2 col-form-label" style="margin-left:350px;">Hora Fin (TARDE):</label><div class="col-sm-3"><input type="time" class="form-control" id="horaFin2Input" name="horaFin2Input" value="{{$turno->horaFin2}}"></div>
                @endif
            </div>
